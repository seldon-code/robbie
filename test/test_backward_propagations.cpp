...
void test_backward_propagation( Robbie::Layer<scalar> * layer, const Robbie::Vector<scalar> & x0, size_t output_size )
{
    ...
    // Then dE/dY = (1, ..., 1)^T
    auto output_error = Robbie::Vector<scalar>::Ones( output_size );
...
TEST_CASE( "Test_BackwardPropagations" )
{
    ...
    Robbie::Vector<scalar> x0 = Robbie::Vector<scalar>::Random( n_input );
    ...
    auto activation_layer_relu = Robbie::ActivationLayer<scalar, Robbie::ActivationFunctions::ReLU<scalar>>();
    test_backward_propagation( &activation_layer_relu, x0, n_input );

    fmt::print( "Test Tanh layer\n" );
    auto activation_layer_tanh = Robbie::ActivationLayer<scalar, Robbie::ActivationFunctions::Tanh<scalar>>();
